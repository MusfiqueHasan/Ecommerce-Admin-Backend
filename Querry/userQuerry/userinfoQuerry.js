...
async function getUserInfo(userId) {
	const sqlQuerry = `SELECT first_name,last_name,user_name,phone_number,profile_img,division,city,address,country,user_role,email FROM user_info WHERE id = ${userId}`;
...
async function updateAddress(country, city, division, address, userId) {
	const sqlQuery = `UPDATE user_info SET country=? ,city=?, division=?,address = ?, updated_date = ? WHERE id = ?`;
	const userData = [country, city, division, address, getTimeStamp(), userId];
